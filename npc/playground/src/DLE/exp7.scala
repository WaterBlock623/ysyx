...
class ParsePs2() extends Module {
  ...
  val dataReg = RegInit(0.U(10.W))
  dataReg := dataReg | io.ps2Dat << dataCntReg
  io.data := dataReg(8, 1)
  io.valid := ~dataCntReg.orR
}
...
class Exp7() extends Module {
  ...
  val notUp = ps.io.data =/= "hf0".U && dataReg =/= "hf0".U
  val keyDownReg = RegEnable(notUp, false.B, validPosEdge) 
  ...
  val cntReg = RegEnable(cntReg + (notUp && (ps.io.data =/= dataReg)).asUInt, 0.U(8.W), ps.io.valid)
