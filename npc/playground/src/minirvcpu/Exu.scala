...
class AluBase(
  ...
  import AluOpEnum._
  io.out := MuxLookup(io.in.aluOp, addResult)(
    Seq(
...
class Exu(
  ...
  // 连接Alu输入
  val src2 = MuxLookup(ctrlSig.aluIn2Sel, io.iduIn.imm)(
    Seq(
  ...
  io.exuOut.aluResult := MuxLookup(ctrlSig.exuOutSel, outTable.head._2)(
    outTable
  )
}
