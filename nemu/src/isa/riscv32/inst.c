...
#define BRANCH_WHEN(cond) \
  ...
  INSTPAT_START();
  // RVC
  INSTPAT("???????????????? 000 ? ?? ??? ?? ??? 00", c_addi4spn , CI_CADDI4SPN, \
      printf("rd=%u, imm=0x%x, x2=0x%x", rd, imm, R(2)); \
      R(rd) = imm + R(2));
  ...
  INSTPAT("???????????????? 010 ? 00 010 ?? ??? 01", c_addi16sp , CI_CADDI16SP, \
      R(2) = imm + R(2));
  INSTPAT("???????????????? 010 ? ?? ??? ?? ??? 01", c_lui , CI_CLUI, \
      R(rd) = imm);
  ...
  INSTPAT("???????????????? 100 ? 10 ??? ?? ??? 01", c_andi , CB_CLIADDI, \
		  R(rd) = src1 & imm); 
  INSTPAT("???????????????? 100 0 11 ??? 00 ??? 01", c_sub , CR_CNONE, \
		  R(rd) = R(rd) - src2); 
  INSTPAT("???????????????? 100 0 11 ??? 01 ??? 01", c_xor , CR_CNONE, \
		  R(rd) = R(rd) ^ src2); 
  INSTPAT("???????????????? 100 0 11 ??? 10 ??? 01", c_or , CR_CNONE, \
		  R(rd) = R(rd) | src2); 
  INSTPAT("???????????????? 100 0 11 ??? 11 ??? 01", c_and , CR_CNONE, \
		  R(rd) = R(rd) & src2); 
  ...
  INSTPAT("???????????????? 010 ? ?? ??? ?? ??? 10", c_lwsp , CI_CLSWSP, \
  ...
  INSTPAT("???????????????? 100 1 ?? ??? ?? ??? 10", c_add , CR_CNONE, \
      R(rd) = src1 + src2);
  INSTPAT("???????????????? 110 ? ?? ??? ?? ??? 10", c_swsp , CSS_CLSWSP, \
      vaddr_t addr = R(2) + imm; \
