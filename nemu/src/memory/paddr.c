...
inline uint8_t* guest_to_host(paddr_t paddr) { return pmem + paddr - CONFIG_MBASE; }
inline paddr_t host_to_guest(uint8_t *haddr) { return haddr - pmem + CONFIG_MBASE; }
...
bool try_paddr_read(paddr_t addr, int len, word_t *dest) {
  if (in_pmem(addr)) {
    *dest = pmem_read(addr, len);
    return true;
  }
...
bool try_paddr_write(paddr_t addr, int len, word_t* data) {
  if (in_pmem(addr)) { 
    pmem_write(addr, len, *data); 
    return true; 
  }
